...
\[
\begin{matrix}
\infer[\mathrm{DR}]{ t \in \mathbf{Read} \mathbin{!} ( p , A ) }{ \delta ( \delta ( p , A ) , t ) \ne \emptyset } & \infer[\mathrm{reads}]{ \mathbf{Read} \mathbin{!} ( r , C ) \subseteq \mathbf{Read} \mathbin{!} ( p , A ) }{ \delta ( \delta ( p , A ) , C ) \ne \emptyset & C \Rightarrow_{P^\prime}^\ast \varepsilon } \\
\\

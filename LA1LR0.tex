...
\begin{align}
LA ( q , [ A \to \omega ] ) \triangleq \set*{ t \in T^\prime \mid S^\prime \xRightarrow[\mathbf{rm}]{}_{P^\prime}^+ \alpha A t z , \alpha \omega : q_0 \to q , \alpha \in ( V^\prime )^\ast , z \in ( T^\prime )^\ast } .
\end{align}
\end{theorem}

...
\]
Since \(\$\notin T\) and the only role of the augmented production
\([S'\to S\$]\) is to create the initial and final LR(0) states, this is
equivalent to \(S\Rightarrow_P^* z\).  Hence
...
are precisely \(\mathsf{Read}(p,A)\), and hence they are included in
\(\mathsf{Follow}(p,A)\) by the rule \(\mathrm{Read}\).
...
\exists\alpha,z\;.
S'\xRightarrow[\mathbf{rm}]{}_{P'}{}^*\alpha A tz
\text{ and }\alpha\omega:q_0\to q \\
&\quad\Longleftrightarrow
\exists p\in Q\;.
\omega:p\to q,\ \delta(p,A)\ne\emptyset,
\text{ and }t\in\mathsf{Follow}(p,A) \\
&\quad\Longleftrightarrow
\exists p\in Q\;.
\omega:p\to q,\ \delta(p,A)\ne\emptyset,
\text{ and }t\in\mathbf{Follow}\mathbin{!}(p,A).
\end{aligned}
\]
The middle equivalence just cuts the path \(\alpha\omega:q_0\to q\) at the point
reached after \(\alpha\); conversely, concatenating \(\alpha:q_0\to p\) and
\(\omega:p\to q\) recovers \(\alpha\omega:q_0\to q\).  This is precisely the
formula claimed in the theorem.
\end{proof}
